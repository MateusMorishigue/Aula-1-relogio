...
int horas = 0, minutos = 0, segundos = -1, dias = 0, tempo = 5;
    ...
    printf("\t\333 Escolha o alarme: (HH MM SS)\t\t\t\333\n"); //Aqui a pessoa � solicitada para digitar tres numeros separados
    printf("\t  ");
    scanf("%d %d %d",&h, &m, &s); //O compilador l� 3 numeros separados e coloca em tres variaveis
    ...
    while(1){
        ...
        if(h == horas && m == minutos && s == segundos){
            ...
            PlaySound(TEXT("siren1.wav"),NULL,SND_ASYNC);
            Sleep(tempo*1000);
